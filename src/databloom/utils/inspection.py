...
def profile_dataframe(df: pd.DataFrame) -> DataFrameProfile:
    ...
    for col_name in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            if (
                unique <= settings.inspection.categorical_max_unique
                and unique < len(df) * settings.inspection.categorical_unique_ratio
            ):
    ...
    if len(profile.temporal_cols) >= 1 and len(profile.numeric_cols) >= 1:
        profile.suggested_chart_types = list(settings.inspection.temporal_chart_suggestions)
    elif len(profile.categorical_cols) >= 1 and len(profile.numeric_cols) >= 1:
        profile.suggested_chart_types = list(settings.inspection.categorical_chart_suggestions)
    elif len(profile.numeric_cols) >= 2:
        profile.suggested_chart_types = list(settings.inspection.numeric_only_chart_suggestions)

    # Suggest layout
    if (
        profile.col_count <= settings.inspection.simple_max_cols
        and profile.row_count <= settings.inspection.simple_max_rows
    ):
        profile.suggested_layout = "simple"
    elif profile.col_count <= settings.inspection.summary_detail_max_cols:
